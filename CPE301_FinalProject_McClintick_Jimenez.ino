...
void running(){ //function for the running state
  *port_a |= 0x01;
  *port_a &= 0xFD;
  *port_a &= 0xF7;
  *port_a &= 0xFB;
...
void idle(){ //function for the idle state
  *port_a &= 0xFE;
  *port_a &= 0xFD;
  *port_a |= 0x08;
  *port_a &= 0xFB;
...
void error(){ //function for the error state
  *port_a &= 0xFE;
  *port_a |= 0x02;
  *port_a &= 0xF7;
  *port_a &= 0xFB;
...
void tempHumidity(){ //function for temp and humidity
...
void stepperOn(){ //function for stepper motor
...
void fanOn(){ //function to turn fan on
  analogWrite(enM, 255);
  *port_b |= 0x02;
  *port_b |= 0x04;
}

void fanOff(){ //function to turn the fan off
  *port_b &= 0xFD;
  *port_b &= 0xFB;
}

void RTCUpdate(){ //function to print time from rtc to the serial monitor
...
void my_delay(unsigned int delay) //function to create a delay
{
  // calc period
...
void U0init(int U0baud) //function to set up serial monitor
{
